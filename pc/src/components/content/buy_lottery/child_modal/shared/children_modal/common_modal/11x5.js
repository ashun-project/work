function num3010101 (betballs) {
    ...
    allResults.forEach(item => {
        let sp = item.split(',');
        if (sp[0] !== sp[1] && sp[0] !== sp[2] && sp[1] !== sp[2]) results.push(item);
    })
    return results
}
...
function num3050201 (betballs) {
    ...
    betballs = betballs.split(',');
    betballs.forEach(item => {
        if (num.some(vvv => vvv === item) && !results.some(vvv => vvv === item)) {
            results.push(item);
        }
    });
    return results;
...
function num3050203 (betballs) {
    ...
    betballs.forEach(item => {
        ...
        // 去掉num没有匹配到的数据或是重复的数据
        for (var i = 0; i < sp.length; i++) {
            if (!num.some(vvv => vvv === sp[i]) || repeate.some(vvv => vvv === sp[i])) {
                spRepeat = false;
                break;
            }
            repeate.push(sp[i]);
