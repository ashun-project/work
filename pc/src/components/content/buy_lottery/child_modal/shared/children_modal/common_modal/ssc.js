...
function num4040201(betballs) { //后三组三 - 组三
    // console.log(betballs);
    var results = [];
    var balls = betballs.split(",");
    for (var i = 0, len1 = Math.pow(2, balls.length); i < len1; i++) {
...
function num4070202(betballs) {
    ...
    betballs = betballs.split(' ');
    betballs.forEach(item => {
        if (item.length === 2 && !item.match(/^([0-9])\1$/) && !results.some(value => value === item)) {
            results.push(item);
        }
    });
    return results;
...
function num4100201(betballs) {
    ...
    for (var i = 0, len1 = posrets.length; i < len1; i++) {
        ...
        if (betgroup1.length == 0 || betgroup2.length == 0 || betgroup3.length == 0) continue;
        results = [];
        result = [];
        groups = [betgroup1.split(","), betgroup2.split(","), betgroup3.split(",")];
        doExchange(groups, 0);
        //拆解号码所在位置
...
function num4100301(betballs) {
    ...
    for (var i = 0, len1 = posrets.length; i < len1; i++) {
        ...
        if (betgroup1.length == 0 || betgroup2.length == 0 || betgroup3.length == 0 || betgroup4.length == 0) continue;
        results = [];
        result = [];
        groups = [betgroup1.split(","), betgroup2.split(","), betgroup3.split(","), betgroup4.split(",")];
        doExchange(groups, 0);
        //拆解号码所在位置
        for (var j = 0, len2 = results.length; j < len2; j++) {
            var balls = results[j].split(",");
...
function num4100102(betballs) {
    ...
    var ballgroups = betballs.split("|");
    var balls = ballgroups[0].split(",");
    if (ballgroups.length < 2 || balls.length < 2 || ballgroups.some(item => !item)) {
        return [];
    }
    for (var i = 0, len1 = Math.pow(2, balls.length); i < len1; i++) {
        var cur = [];
    ...
    for (var i = 0, len2 = nums.length; i < len2; i++) {
        if (nums[i].length == 2 && !numResult.some(item => item === nums[i]) && /^[0-9]*$/.test(nums[i])) {
            count++;
...
function num4100202(betballs) {
    ...
    var numResult = [];
    for (var i = 0, len2 = nums.length; i < len2; i++) {
        if (nums[i].length == 3 && !numResult.some(item => item === nums[i]) && /^[0-9]*$/.test(nums[i])) {
            count++;
            numResult.push(nums[i])
        };
    }
