...
function getZodiac(time) {
	...
	function addCode(temp, i, j) {
		var code = parseInt(temp[i][j]) + 12;
		if (code <= 49) {
			temp[i].push(code < 10 ? '0' + code : '' + code);
			addCode(temp, i, j + 1)
		}
	}

	let arr2 = ['鼠', '牛', '虎', '兔', '龙', '蛇', '马', '羊', '猴', '鸡', '狗', '猪'];

	return arr2.map((item, idx) => {
		return {
			label: item,
			data: arr[idx]
		}
